...
export class MinimumEnclosingHilbertBall {
  ...
  shuffledPoints(points) {
    ...
    console.log("before shuffled: ",shuffled);
  ...
  makeBallOnePoint(points, p) {
   ...
   makeBottomLeftMost(p, q) {
    ...
    let ballCenter = new Site (center.x, center.y, this.omega, 'red');
    ballCenter.setDrawSpokes(false);
    let ball = new HilbertBall(ballCenter, radius);
    ball.setBoundaryColor('red');
    return ball;
  }
...
export class ForwardFunkBall extends Site {
  /* (site:Site, radius:Float, color:String, penWidth:Float) */
  constructor(site, ballRadius = 1, penWidth = 1, boundaryColor = null) {
      super(site);
      this.ballRadius = ballRadius;
...
export class ReverseFunkBall extends Site {
  /* (site:Site, radius:Float, color:String, penWidth:Float) */
  constructor(site, ballRadius = 1, penWidth = 1, boundaryColor = null) {
      super(site);
      this.ballRadius = ballRadius;
      this.pointsOnBall = getPointsOnReverseFunkBall(this, this.ballRadius);
      this.boundaryColor = boundaryColor;
      this.polygon = new ConvexPolygon(this.pointsOnBall, this.boundaryColor, penWidth);
  }
...
export class ThompsonBall extends Site {
  /* (site:Site, radius:Float, color:String, penWidth:Float) */
  constructor(site, ballRadius = 1, penWidth = 1, boundaryColor = null) {
      super(site);
      this.ballRadius = ballRadius;
      ...
      this.polygon = polygon1.createPolygonIntersection(polygon2)
      this.polygon.setColor(this.boundaryColor);
      this.polygon.setShowVertices(false);
  }
  ...
  computeHilbertBall() { 
    const numAttempts = 100; // Number of perturbation attempts
    const epsilon = 0.01; // Base perturbation value

    let pointsOnBall1 = getPointsOnReverseFunkBall(this, this.ballRadius);
    let polygon1 = new ConvexPolygon(pointsOnBall1, this.boundaryColor, this.polygon.penWidth);

    let pointsOnBall2 = getPointsOnForwardFunkBall(this, this.ballRadius);      
    let polygon2 = new ConvexPolygon(pointsOnBall2, this.boundaryColor, this.polygon.penWidth);

    // Array to store all polygon candidates
    let candidates = [];

    // Original intersection without perturbation
    let originalCandidate = polygon1.createPolygonIntersection(polygon2);
    if (originalCandidate) candidates.push(originalCandidate);

    // Generate perturbation-based candidates in a loop
    for (let i = 1; i <= numAttempts; i++) {
        const direction1 = i % 2 === 0 ? 1 : -1; // Alternate directions
        const direction2 = (i + 1) % 2 === 0 ? -1 : 1;

        let perturbedPolygon1 = perturbPolygon(polygon1, epsilon * i, direction1);
        let perturbedPolygon2 = perturbPolygon(polygon2, epsilon * (i / 2), direction2);

        let candidate = perturbedPolygon1.createPolygonIntersection(perturbedPolygon2);
        if (candidate) candidates.push(candidate);
    }

    // Find the polygon with the largest number of vertices
    let bestPolygon = candidates.reduce((maxPolygon, currentPolygon) => 
        currentPolygon.vertices.length > maxPolygon.vertices.length ? currentPolygon : maxPolygon, 
        candidates[0]);

    // Set the final polygon with the most vertices
    this.polygon = bestPolygon;
    this.polygon.setColor(this.boundaryColor);
    this.polygon.setShowVertices(false);
}
...
export class MultiBall extends Site {
    ...
    addBall(type, ball) {
        this.balls.push({ type, ball });
    }

    setBoundaryColor(type, color) {
        const targetBall = this.balls.find(b => b.type === type);
        if (targetBall) targetBall.ball.setBoundaryColor(color);
    }
    ...
    draw(ctx) {
        // Draw the site only once
        super.draw(ctx);

        // Draw all associated balls
        this.balls.forEach(({ ball }) => 
        {
          ball.polygon.setShowVertices(false);
          ball.polygon.draw(ctx);
        }
      );
    }

    updateFromCollapsibles() {
        // Update each ball based on the current collapsible inputs
        this.balls.forEach(({ type }) => {
            const { colorInputId, radiusInputId } = this.getPropertyElements(type);
            this.setBoundaryColor(type, document.getElementById(colorInputId).value);
            this.setBallRadius(type, parseFloat(document.getElementById(radiusInputId).value));
        });
    }

    computeMultiBall() {
        // Recreate each ball using the updated site but with the old properties.
        this.balls = this.balls.map(({ type, ball }) => {
            const newBall = this.createBallOfType(type, this);
            this.copyBallProperties(ball, newBall); // Copy old properties to the new ball
            return { type, ball: newBall };
        });

    }

    copyBallProperties(oldBall, newBall) {
        newBall.setBallRadius(oldBall.ballRadius);
        newBall.setBoundaryColor(oldBall.boundaryColor);
        newBall.setPenWidth(oldBall.polygon.penWidth);
    }

    getPropertyElements(type) {
        // Map each ball type to its corresponding input elements
        switch (type) {
            case "HilbertBall":
